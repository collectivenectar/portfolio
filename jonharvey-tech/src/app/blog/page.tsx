'use client'
import { listAllPosts, getBlogIndex, BlogIndex, BlogPost } from '../services/pinata'; // Fetch metadata for all posts
...
export default function BlogLandingPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const index : BlogIndex = await getBlogIndex();
      if(index) {
    ...
    return (
      <div className="w-full h-screen text-white font-Outfit text-center text-3xl mt-12">
        <ul className="px-4 md:w-1/2 mx-auto pt-20">
          {[...Array(5)].map((_, index) => (
            <li key={index} className="h-fit border border-white rounded-3xl my-4 animate-pulse">
